...
const STATUS_MAP: Record<string, { label: string; color: string; icon: any }> = {
  ...
  resolved: { label: "Resolvido", color: "text-emerald-400 bg-emerald-500/10 border-emerald-500/20", icon: CheckCircle },
};

const RequestsPage = () => {
  const [tab, setTab] = useState<"pedidos" | "reports">("pedidos");
  ...
  useEffect(() => {
    Promise.all([fetchRequests(), fetchReports()]).finally(() => setLoading(false));
  }, [fetchRequests, fetchReports]);

  // Realtime
  useEffect(() => {
    const channel = supabase
      .channel("admin-requests-reports")
      .on("postgres_changes", { event: "*", schema: "public", table: "content_requests" }, () => fetchRequests())
  ...
  const addToContent = async (req: ContentRequest) => {
    ...
    await supabase.from("content").upsert({
      tmdb_id: req.tmdb_id, content_type: req.content_type, title: req.title,
      original_title: req.original_title, poster_path: req.poster_path,
      backdrop_path: req.backdrop_path, overview: req.overview,
      release_date: req.release_date, vote_average: req.vote_average || 0, status: "published",
    }, { onConflict: "tmdb_id,content_type" });
    await supabase.from("video_cache").upsert({
      tmdb_id: req.tmdb_id, content_type: req.content_type, video_url: url,
      video_type: url.includes(".m3u8") ? "m3u8" : "mp4", provider: "manual", audio_type: "legendado",
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs sm:text-sm text-muted-foreground mt-1">Gerenciar pedidos e problemas reportados</p>
      </div>
      ...
      <div className="flex gap-2">
        <button
          ...
          className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors flex items-center gap-2 ${
            tab === "pedidos" ? "bg-primary/15 text-primary border border-primary/20" : "bg-white/5 text-muted-foreground border border-white/10 hover:bg-white/10"
          }`}
        >
          <MessageSquare className="w-3.5 h-3.5" />
          Pedidos
          {pendingRequests > 0 && <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-primary text-primary-foreground font-bold">{pendingRequests}</span>}
        </button>
        <button
          onClick={() => setTab("reports")}
          className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors flex items-center gap-2 ${
            tab === "reports" ? "bg-primary/15 text-primary border border-primary/20" : "bg-white/5 text-muted-foreground border border-white/10 hover:bg-white/10"
          }`}
        >
          <Flag className="w-3.5 h-3.5" />
          Reports
          {pendingReports > 0 && <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-destructive text-destructive-foreground font-bold">{pendingReports}</span>}
        </button>
      </div>
        ...
        requests.length === 0 ? (
          ...
          <div className="space-y-2">
            {requests.map((req) => {
              ...
              const Icon = st.icon;
              return (
                <button key={req.id} onClick={() => { setSelected(req); setVideoUrl(""); }}
          ...
          <div className="space-y-2">
            {reports.map((r) => (
              <button key={r.id} onClick={() => setSelectedReport(r)}
                className="w-full text-left p-4 rounded-xl bg-white/[0.03] border border-white/5 hover:bg-white/[0.06] hover:border-white/10 transition-colors">
      ...
      {selected && (
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4" onClick={() => setSelected(null)}>
          ...
          <div className="relative w-full sm:max-w-2xl max-h-[90vh] sm:max-h-[85vh] bg-card/90 backdrop-blur-2xl border border-white/10 overflow-hidden animate-scale-in flex flex-col rounded-t-3xl sm:rounded-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex items-start justify-between p-4 sm:p-5 -mt-12 relative z-10">
              <div className="flex gap-4">
                {selected.poster_path && <img src={posterUrl(selected.poster_path)} alt="" className="w-16 h-24 sm:w-20 sm:h-28 rounded-xl object-cover shadow-xl flex-shrink-0" />}
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-5 pt-0 space-y-4">
              {selected.overview && <p className="text-xs sm:text-sm text-muted-foreground leading-relaxed">{selected.overview}</p>}
              <div className="p-3 rounded-xl bg-white/[0.03] border border-white/5">
                <p className="text-xs text-muted-foreground"><strong className="text-foreground">Pedido por:</strong> {selected.requester_name}</p>
                <p className="text-xs text-muted-foreground mt-1"><strong className="text-foreground">Data:</strong> {new Date(selected.created_at).toLocaleString("pt-BR")}</p>
              </div>
              <div>
                <label className="text-xs font-medium text-muted-foreground mb-1.5 block">Link MP4 do vídeo</label>
                <div className="flex gap-2">
                  <input type="text" value={videoUrl} onChange={(e) => setVideoUrl(e.target.value)} placeholder="https://cdn.exemplo.com/video.mp4"
                  ...
                  <button onClick={() => tryResolveVideo(selected)} disabled={resolving}
              ...
              <div className="flex flex-wrap gap-2">
                {selected.status !== "completed" && (
                  <button onClick={() => addToContent(selected)} className="flex-1 py-2.5 rounded-xl bg-primary text-primary-foreground text-sm font-semibold hover:bg-primary/90 transition-colors">Publicar no Site</button>
                )}
                {selected.status === "pending" && (
                  <button onClick={() => updateStatus(selected.id, "rejected")} className="px-4 py-2.5 rounded-xl bg-destructive/10 text-destructive text-sm font-medium hover:bg-destructive/20 transition-colors">Rejeitar</button>
                )}
              </div>
            </div>
          </div>
      ...
      {selectedReport && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={() => setSelectedReport(null)}>
          ...
          <div className="relative w-full max-w-lg bg-card border border-white/10 rounded-2xl p-6" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setSelectedReport(null)} className="absolute top-4 right-4 text-muted-foreground hover:text-foreground"><X className="w-5 h-5" /></button>
            <div className="flex items-center gap-3 mb-5">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${selectedReport.status === "pending" ? "bg-amber-500/20" : "bg-green-500/20"}`}>
                {selectedReport.status === "pending" ? <Clock className="w-5 h-5 text-amber-500" /> : <CheckCircle className="w-5 h-5 text-green-500" />}
              </div>
              <div>
                <h3 className="font-display text-lg font-bold">{selectedReport.title}</h3>
                <p className="text-muted-foreground text-xs">{selectedReport.content_type === "movie" ? "Filme" : "Série"} • TMDB #{selectedReport.tmdb_id}</p>
              </div>
            </div>
            <div className="space-y-4">
              ...
              <div className="flex gap-2 pt-2">
                ...
                {selectedReport.status === "pending" && (
                  <button onClick={() => handleResolveReport(selectedReport)} disabled={resolving}
                    ...
                    {resolving ? <div className="w-4 h-4 border-2 border-green-400 border-t-transparent rounded-full animate-spin" /> : <CheckCircle className="w-4 h-4" />}
                    Resolvido
                  </button>
                )}
              </div>
